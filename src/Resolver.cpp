...
void Resolver::resolveFunction(
    ...
    std::vector<std::shared_ptr<Statement::Stmt>> body, FType type) {
...
std::any
Resolver::visitExpressionStmt(std::shared_ptr<Statement::Expression> stmt) {
...
std::any Resolver::visitVariableExpr(std::shared_ptr<Variable> expr) {
  if (!scopes.empty()) {
    ...
    if (elem != currentScope.end() && elem->second == false) {
      Debug::error(expr->name,
                   "Can't read local variable in this own initializer.");
    }
  }
...
std::any
Resolver::visitFunctionStmt(std::shared_ptr<Statement::Function> stmt) {
  declare(stmt->name);
  define(stmt->name);
