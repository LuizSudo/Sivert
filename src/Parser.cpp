...
std::shared_ptr<Expr> Parser::unary() {
  while (match(TokenType::BANG, TokenType::MINUS, TokenType::PLUS_PLUS,
    ...
    if (previous().lexeme == "++" || previous().lexeme == "--") { // to optimize
      Debug::error(
          previous(),
          "Invalid operator: postfix increment/decrement followed by prefix.");
    }
...
template <class... T> void Parser::matchVoid(T... types) {
  assert((... && std::is_same_v<T, TokenType>));
  if ((... || check(types))) {
    advance();
  }
...
bool Parser::isAtEnd() { return peek().type == TokenType::LANG_EOF; }
...
std::shared_ptr<Statement::Stmt> Parser::includeStatement() {
  ...
  Token path = consume(TokenType::STRING,
                       "Expect 'path' as a string inside 'include' statement.");
  consume(TokenType::RIGHT_PAREN,
          "Expect ')' after path in 'include' statement.");
  matchVoid(TokenType::SEMICOLON);

  if (std::find(includedFiles.begin(), includedFiles.end(), path.lexeme) !=
      includedFiles.end()) {
  ...
  std::vector<std::shared_ptr<Statement::Stmt>> includedStatements =
      includedParser.parse();
