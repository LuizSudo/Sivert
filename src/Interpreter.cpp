...
Interpreter::Interpreter() {
  ...
  global->define("random", std::shared_ptr<Rand>{});
  global->define("getenv", std::shared_ptr<GetEnv>{});
  global->define("tostring", std::shared_ptr<ToString>{});
  global->define("version", std::shared_ptr<Args>{});
}
...
std::string Interpreter::stringify(const std::any &object) {
  ...
  if (object.type() == typeid(std::string)) {
    ...
    // Substituir as sequências "\n" e "\r" por novas linhas e retornos de carro
    // reais
    size_t pos;
...
std::any
Interpreter::visitIncludeStmt(std::shared_ptr<Statement::Include> stmt) {
  if (stmt->keyword.type != TokenType::IMPORT) {
    return {};
  }
  return {};
}
